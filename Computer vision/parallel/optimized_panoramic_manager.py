...
class PanoramicCameraManager:
    ...
    def stitch_panoramic(self, img1: np.ndarray, img2: np.ndarray) -> Optional[np.ndarray]:
        ...
        try:
            ...
            # Create panoramic image with proper blending
            result = self._create_panoramic_blend(img1, img2, homography)
            
            # Calculate frame processing time
            frame_processing_time = time.time() - stitch_start
            self.stitch_times.append(frame_processing_time)
            ...
            logger.debug(f"Panoramic stitch successful: {len(matches)} matches, {frame_processing_time:.3f}s")
    ...
    def _create_panoramic_blend(self, img1: np.ndarray, img2: np.ndarray, homography: np.ndarray) -> np.ndarray:
        """Create seamless panoramic blend using homography"""
        h1, w1 = img1.shape[:2]
        h2, w2 = img2.shape[:2]
        ...
        # Ensure img1 fits within result bounds
        img1_end_y = min(img1_y_offset + h1, output_height)
        img1_end_x = min(img1_x_offset + w1, output_width)
        
        if img1_y_offset >= 0 and img1_x_offset >= 0:
            result[img1_y_offset:img1_end_y, img1_x_offset:img1_end_x] = img1[:img1_end_y-img1_y_offset, :img1_end_x-img1_x_offset]
        ...
        # Mark regions where each image contributes
        if img1_y_offset >= 0 and img1_x_offset >= 0:
            mask1[img1_y_offset:img1_end_y, img1_x_offset:img1_end_x] = 1.0
        
        mask2[warped_img2.sum(axis=2) > 0] = 1.0
        
        # Find overlap region
        overlap = (mask1 > 0) & (mask2 > 0)
        
        if np.any(overlap):
            # Create smooth blending weights using linear blending
            # Distance from left edge for horizontal blending
            x_coords = np.arange(output_width)
            y_coords = np.arange(output_height)
            X, Y = np.meshgrid(x_coords, y_coords)
            
            # Find overlap boundaries
            overlap_left = np.min(X[overlap]) if np.any(overlap) else 0
            overlap_right = np.max(X[overlap]) if np.any(overlap) else output_width
            overlap_width = overlap_right - overlap_left
            
            if overlap_width > 0:
                # Create horizontal blending weights
                alpha = np.zeros((output_height, output_width), dtype=np.float32)
                alpha[overlap] = (X[overlap] - overlap_left) / overlap_width
                alpha = np.clip(alpha, 0, 1)
                
                # Apply blending in overlap region
                for c in range(3):  # For each color channel
                    overlap_blend = ((1 - alpha) * result[:, :, c] + alpha * warped_img2[:, :, c])
                    result[overlap, c] = overlap_blend[overlap].astype(np.uint8)
    ...
    def _blend_images(self, base_img: np.ndarray, overlay_img: np.ndarray, 
        ...
        """Advanced blending to create seamless panoramic view"""
        ...
        # Mark regions where each image has valid pixels
        mask_base[img1_y_offset:img1_y_offset + h1, img1_x_offset:img1_x_offset + w1] = 255
        mask_overlay[overlay_img.sum(axis=2) > 0] = 255
        ...
        if np.any(overlap_mask):
            ...
            # Calculate blending weights (closer to image center = higher weight)
            alpha_base = dist_base / total_dist
            alpha_overlay = dist_overlay / total_dist
            
            # Apply smooth blending in overlap region
            overlap_y, overlap_x = np.where(overlap_mask)
            
            for y, x in zip(overlap_y, overlap_x):
                # Weighted blend based on distance from image edges
                weight_base = alpha_base[y, x]
                weight_overlay = alpha_overlay[y, x]
                
                # Blend the pixels
                result[y, x] = (weight_base * result[y, x] + 
                               weight_overlay * overlay_img[y, x]).astype(np.uint8)
