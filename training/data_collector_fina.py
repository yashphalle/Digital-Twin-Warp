#!/usr/bin/env python3
"""
Threaded Data Collection for Custom Training with Duplicate Detection
Collects raw 4K + fisheye-corrected 2K images from all 11 warehouse cameras
Only saves images that are different from the last saved image
"""
...
class CameraCollectionThread(threading.Thread):
    """Individual camera collection thread with duplicate detection"""
    ...
    def __init__(self, camera_id: int, output_dir: str, gui_callback=None):
        ...
        self.check_interval = 6000  # 5 minute at 20fps (changed from frame_skip_count)
        self.force_save_interval = 10 * 60  # 10 minutes in seconds
        self.raw_jpeg_quality = Config.RAW_IMAGE_QUALITY
        self.corrected_jpeg_quality = Config.CORRECTED_IMAGE_QUALITY
        
        # Duplicate detection settings
        self.last_saved_image_hash = None
        self.similarity_threshold = 5  # Same as duplicate detection script
        ...
        # Statistics (enhanced for duplicate tracking)
        self.stats = {
            'frames_processed': 0,
            'images_saved': 0,
            'images_skipped': 0,  # New: track skipped duplicates
            'force_saves': 0,     # New: track force saves
            'last_save_time': None,
            'last_save_reason': None,  # New: reason for last save/skip
        ...
        logger.info(f"Camera {camera_id} thread initialized with duplicate detection")
    ...
    def _connect_camera(self) -> bool:
        """Connect to camera with retry logic and proper timeout settings like working scripts"""
        max_retries = 3
        retry_delay = 5
        ...
        for attempt in range(max_retries):
            try:
                ...
                if self.cap.isOpened():
                    ...
                    # Set buffer size and timeout settings (like working scripts)
                    ...
                    logger.info(f"🔧 Camera {self.camera_id}: Buffer size and timeouts set")

                    # Test read with retry logic (like working scripts)
                    logger.info(f"🔧 Camera {self.camera_id}: Testing frame read with retries...")
                    stable_reads = 0
                    for test_attempt in range(3):
                        ret, frame = self.cap.read()
                        if ret and frame is not None:
                            stable_reads += 1
                            logger.info(f"✅ Camera {self.camera_id}: Test read {test_attempt + 1}/3 successful - Frame shape: {frame.shape}")
                        else:
                            logger.warning(f"⚠️ Camera {self.camera_id}: Test read {test_attempt + 1}/3 failed - ret: {ret}, frame: {frame is not None}")
                        time.sleep(0.1)
                    
                    if stable_reads >= 2:  # At least 2/3 successful reads
                        self.stats['connection_status'] = 'Connected'
                        logger.info(f"🎉 Camera {self.camera_id}: Connected successfully ({stable_reads}/3 stable reads)")
                        return True
                    else:
                        logger.warning(f"❌ Camera {self.camera_id}: Unstable connection ({stable_reads}/3 successful reads)")
                        self.cap.release()
                        self.cap = None
                else:
    ...
    def _calculate_frame_hash(self, frame: np.ndarray) -> imagehash.ImageHash:
        """Calculate perceptual hash for duplicate detection using same method as duplicate script"""
        try:
            # Apply fisheye correction first (to match what gets saved)
            corrected_frame = self.fisheye_corrector.correct(frame)
            
            # Resize to 2K if larger (to match what gets saved)
            ...
            corrected_rgb = cv2.cvtColor(corrected_frame, cv2.COLOR_BGR2RGB)
            pil_image = Image.fromarray(corrected_rgb)
            
            # Calculate perceptual hash (same as duplicate detection script)
            phash = imagehash.phash(pil_image, hash_size=16)
            return phash
            
        except Exception as e:
            logger.error(f"Camera {self.camera_id}: Hash calculation error: {e}")
            # Return a random hash to trigger save (fail-safe)
            return imagehash.phash(Image.new('RGB', (100, 100)))
    
    def _should_save_frame(self, frame: np.ndarray) -> tuple:
        """
        Decide if frame should be saved based on duplicate detection + timing rules
        Returns: (should_save: bool, reason: str)
        """
        current_time = time.time()
        
        # Always save first image
        if self.last_saved_image_hash is None:
            return True, "First image"
        
        # Force save after 10 minutes regardless
        time_since_last_save = current_time - self.last_force_save_time
        if time_since_last_save >= self.force_save_interval:
            return True, f"Force save ({time_since_last_save/60:.1f}min elapsed)"
        ...
        try:
            ...
            diff = abs(current_hash - self.last_saved_image_hash)
            
            if diff > self.similarity_threshold:
                return True, f"Different image (diff: {diff})"
            else:
                return False, f"Duplicate image (diff: {diff})"
                
        except Exception as e:
            logger.error(f"Camera {self.camera_id}: Error in should_save_frame: {e}")
            # Fail-safe: save on error
            return True, f"Error in comparison - saving anyway: {e}"
    ...
    def _save_images(self, raw_frame: np.ndarray) -> bool:
        ...
        try:
            ...
            self.last_saved_image_hash = self._calculate_frame_hash(raw_frame)
            self.last_force_save_time = time.time()
            ...
            # Track if this was a force save
            if "Force save" in self.stats['last_save_reason']:
                self.stats['force_saves'] += 1
            ...
            logger.info(f"Camera {self.camera_id}: Saved images #{self.stats['images_saved']} - {self.stats['last_save_reason']}")
            return True
    ...
    def run(self):
        """Main collection loop with duplicate detection"""
        logger.info(f"🚀 Camera {self.camera_id}: Starting collection thread with duplicate detection")
        self.running = True
        ...
        frame_count = 0
        logger.info(f"🔧 Camera {self.camera_id}: Entering main collection loop with duplicate detection...")
        ...
        while self.running:
            try:
                ...
                # Read frame with retry logic (like working scripts)
                ret, frame = None, None
                read_attempts = 0
                max_read_attempts = 3
                ...
                while read_attempts < max_read_attempts:
                    ...
                    if ret and frame is not None:
                        ...
                        read_attempts += 1
                        logger.debug(f"Camera {self.camera_id}: Read attempt {read_attempts}/{max_read_attempts} failed")
                        time.sleep(0.01)
                
                if not ret or frame is None:
                    logger.warning(f"❌ Camera {self.camera_id}: Failed to read frame after {max_read_attempts} attempts - ret: {ret}, frame: {frame is not None}")
                    self.cap.release()
                    self.cap = None
                    continue
                ...
                # Check if it's time to evaluate frame (every 1 minute = 1200 frames at 20fps)
                if frame_count >= self.check_interval:
                    logger.info(f"🔍 Camera {self.camera_id}: Evaluating frame at {frame_count} frames (1min interval)")
                    
                    # Decide if we should save this frame
                    should_save, reason = self._should_save_frame(frame)
                    self.stats['last_save_reason'] = reason
                    
                    if should_save:
                        logger.info(f"💾 Camera {self.camera_id}: Saving - {reason}")
                        ...
                        logger.info(f"⏭️  Camera {self.camera_id}: Skipping - {reason}")
                        self.stats['images_skipped'] += 1
                    
                    frame_count = 0  # Reset counter regardless
                
                # Brief pause to prevent excessive CPU usage (but don't slow down frame reading too much)
                time.sleep(0.005)  # Reduced from 0.01 to match working scripts pattern
...
class CollectionGUI:
    ...
    def __init__(self):
        ...
        self.root.title("Warehouse Camera Data Collection - With Duplicate Detection")
        ...
        self.total_stats = {
            'total_images': 0,
            'total_skipped': 0,  # New: track skipped images
            'active_cameras': 0,
    ...
    def setup_gui(self):
        ...
        # Main title
        title_label = tk.Label(self.root, text="Warehouse Camera Data Collection - With Duplicate Detection",
                              font=("Arial", 16, "bold"))
        ...
        for camera_id in range(1, 12):  # Cameras 1-11
            ...
            # Info display (enhanced for duplicate tracking)
        ...
        status_frame = tk.Frame(self.root, bg='lightblue', relief='raised', bd=2)
        status_frame.pack(pady=10, fill='x')

        self.status_label = tk.Label(status_frame, text="🚀 DATA COLLECTION WITH DUPLICATE DETECTION RUNNING",
    ...
    def update_camera_display(self, camera_id: int, thumbnail: np.ndarray, stats: Dict):
        ...
        try:
            ...
            # Update info (enhanced for duplicate tracking)
            ...
            info_text += f"Saved: {stats['images_saved']}\n"
            info_text += f"Skipped: {stats['images_skipped']}\n"  # New
            info_text += f"Force: {stats['force_saves']}\n"      # New
            info_text += f"Last: {last_save_str}\n"
            info_text += f"Reason: {stats.get('last_save_reason', 'N/A')[:20]}...\n"  # New
    ...
    def update_stats(self):
        ...
        if hasattr(self, 'collection_threads'):
            ...
            total_skipped = sum(thread.stats['images_skipped'] for thread in self.collection_threads)  # New
            ...
            stats_text = f"Saved: {total_images} | Skipped: {total_skipped} | Active: {active_cameras}/11 | Runtime: {runtime_str}"
    ...
    def start_collection(self):
        ...
        logger.info("🚀 Starting data collection with duplicate detection...")
        ...
        self.status_label.config(text="🎉 DATA COLLECTION WITH DUPLICATE DETECTION ACTIVE",
        ...
        logger.info("🎉 Data collection with duplicate detection started for all cameras")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Threaded warehouse camera data collection with duplicate detection')
    parser.add_argument('--output-dir', type=str, default=Config.RAW_COLLECTION_DIR,
                       help='Output directory for collected images')
    parser.add_argument('--check-interval', type=int, default=1200,
                       help='Number of frames between duplicate checks (default: 1200 = 1min at 20fps)')
    parser.add_argument('--similarity-threshold', type=int, default=5,
                       help='Similarity threshold for duplicate detection (default: 5)')
    parser.add_argument('--force-save-minutes', type=int, default=10,
                       help='Force save after this many minutes regardless (default: 10)')
    ...
    if args.no_gui:
        ...
        logger.info("Starting headless data collection with duplicate detection...")
        ...
        try:
            ...
            for camera_id in range(1, 12):
                ...
                thread.check_interval = args.check_interval
                thread.similarity_threshold = args.similarity_threshold
                thread.force_save_interval = args.force_save_minutes * 60
                threads.append(thread)
                thread.start()
            
            logger.info("All camera threads started with duplicate detection. Press Ctrl+C to stop.")
        ...
        # GUI mode
        logger.info("Starting GUI data collection with duplicate detection...")
