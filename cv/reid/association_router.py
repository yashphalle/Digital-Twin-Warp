    ...
    def enqueue(self, msg: Dict):
        ...
        for t in tracks:
            ...
            if age > 1 and self.ram_map.get(key) == gid:
                ...
                # New or unknown mapping: push to feature extraction (non-blocking)
                try:
                    item = {'camera_id': cid, 'track_id': yid, 'global_id': gid, 'bbox': bbox}
                    if self.feature_in_q.full():
                        _ = self.feature_in_q.get_nowait()
                    ...
                    pass
                # Additionally, honor DB age gate: forward to DB when age >= configured threshold
                try:
                    db_age = int(getattr(self.cfg, 'db_age_threshold', 5))
                except Exception:
                    db_age = 5
                if age >= db_age:
                    try:
