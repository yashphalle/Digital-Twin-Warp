...
from modules.coordinate_mapper import CoordinateMapper  # reuse tested mapper
from modules.color_extractor import ObjectColorExtractor  # enabled for insert-only
    ...
    def __init__(
        ...
        for cid in self.active_cameras:
            try:
                ...
                # Load per-camera calibration from cv/config (module-relative)
                calib_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'config', f'warehouse_calibration_camera_{cid}.json'))
