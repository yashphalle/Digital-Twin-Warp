...
_cv_path = os.path.abspath(os.path.dirname(__file__))
...
from config.runtime import default_config
from buffers.latest_frame_store import LatestFrameStore
from capture.camera_worker import CameraWorker
from detection.detector_worker import DetectorWorker
from tracking.tracker_manager import TrackerManager
from tracking.tracking_orchestrator import TrackingOrchestrator
...
def run_detection_only(active_cameras: List[int], duration_s: float = 60.0, show_gui: bool = False):
    ...
    if show_gui:
        try:
            from gui.gui_display import GridGUIThread
...
def run_tracking(active_cameras: List[int], duration_s: float = 60.0, show_gui: bool = False,
    ...
    from db.mongo_writer import MongoWriterThread
    db_writer = None
    ...
    from reid.redis_client import RedisClient
    from reid.feature_worker import FeatureWorker
    from reid.reid_worker import ReIDWorker
    from reid.redis_writer import RedisWriter
    from reid.association_router import AssociationRouter
    ...
    if getattr(cfg, 'use_batched_tracking', False):
        ...
        repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
        botsort_yaml = os.path.abspath(os.path.join(repo_root, 'cv', 'config', 'warehouse_botsort.yaml'))
