...
class CoordinateMapper:
    ...
    def load_calibration(self, filename=None):
        # Prefer cv/config by default; do not create any directories
        config_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'config'))
        if filename is None:
            filename = os.path.join(
                config_dir,
                f"warehouse_calibration_camera_{self.camera_id}.json" if self.camera_id else "warehouse_calibration.json"
            )
        # Legacy fallback: repo-root/configs if the file isn't found in cv/config
