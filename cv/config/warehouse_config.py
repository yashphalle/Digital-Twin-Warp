...
class WarehouseConfigManager:
    ...
    def __init__(self, config_dir: str | None = None):
        # Read-only by default: do not auto-create directories or files
        if config_dir is None:
            config_dir = os.path.join(os.path.dirname(__file__), "warehouse_configs")
        self.config_dir = config_dir
        self.current_config: Optional[WarehouseConfig] = None
        # No auto-create: _ensure_config_dir() intentionally not called
    ...
    def get_current_config(self) -> WarehouseConfig:
        ...
        if self.current_config is None:
            ...
            try:
                ...
                logger.info("Using built-in default WARP warehouse configuration (read-only; no file write)")
