...
from camera_manager import CPUCameraManager
from fisheye_corrector import OptimizedFisheyeCorrector

# Import configs using relative path
sys.path.append(os.path.join(os.path.dirname(__file__), '..'))
from configs.config import Config
from configs.warehouse_config import get_warehouse_config
    ...
    def _camera_worker(self, camera_id: int):
        ...
        while self.running:
            try:
                ...
                # Queue for detection threads (ORIGINAL)
                ...
                # NEW: Queue for display (if display manager exists)
                if hasattr(self.queue_manager, 'camera_display_queues') and camera_id in self.queue_manager.camera_display_queues:
                    try:
                        ...
                        logger.debug(f"🖥️  Camera {camera_id}: Queued frame for display")
                    except Exception as e:
                        logger.debug(f"🖥️  Camera {camera_id}: Display queue error: {e}")
