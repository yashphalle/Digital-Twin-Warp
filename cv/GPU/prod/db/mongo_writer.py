...
# We rely on app.py having appended the 'cv' dir to sys.path so these imports work
from final.modules.coordinate_mapper import CoordinateMapper  # reuse tested mapper
# from final.modules.color_extractor import ObjectColorExtractor  # color disabled in this build
...
class MongoWriterThread(threading.Thread):
    ...
    def __init__(
        ...
        # Color extraction disabled per request to reduce CPU load
        self._color = None
    ...
    def _color_from_roi(self, cid: int, bbox: List[float]) -> Dict:
        ...
        try:
            info = self._color.extract_dominant_color(roi)
        except Exception:
            info = None
        if not info:
            return {}
        ...
        return {
            'color_rgb': info.get('color_rgb'),
            'color_hsv': info.get('color_hsv'),
            'color_hex': info.get('color_hex'),
            'color_name': info.get('color_name'),
            'color_confidence': info.get('color_confidence'),
                    ...
                    for msg in msgs:
                        ...
                        for t in tracks:
                            try:
                                ...
                                color_fields = {}  # color extraction disabled
